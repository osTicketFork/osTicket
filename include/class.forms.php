...
class FormField {
    ...
    function getConfiguration() {
        if (!$this->_config) {
            ...
            if (@USE_BOOTSTRAP) {
                if (!isset($this->_config['classes']))
                {
                    $this->_config['classes'] = "form-control";
                }
                else {
                    $classes = split(" ", $this->_config['classes']);
                    if (!in_array("form-control", $classes)) {
                        array_unshift($classes, "form-control");
                    }
                    $this->_config['classes'] = implode(" ", $classes);
                }
             }
        }
        return $this->_config;
...
class Widget {
    ...
    function addClass($config = array(), $classes) {
        if (!isset($config['classes']))
        {
            $config['classes'] = implode(" ", (array) $classes);
        }
        else {
            $config['classes'] = implode(" ", array_merge((array) $classes, split(" ", $config['classes'])));
        }
        return $config;
    }
    ...
    function getInlineAttributes($config = array(), $attibutes = array())
    {
        if (isset($config['size'])) {
            $attibutes[] = "size=\"{$config['size']}\"";
        }

        if (isset($config['length']) && $config['length']) {
            $attibutes[] = "maxlength=\"{$config['length']}\"";
        }

        if (isset($config['classes'])) {
            $attibutes[] = 'class="'.$config['classes'].'"';
        }

        if (isset($config['autocomplete'])) {
            $attibutes[] = 'autocomplete="'.($config['autocomplete']?'on':'off').'"';
        }
        ...
        if (isset($config['translatable']) && $config['translatable']) {
            $attibutes[] = 'data-translate-tag="'.$config['translatable'].'"';
        }

        return implode(' ', $attibutes);
    }
...
class TextboxWidget extends Widget {
    ...
    function render($options=array(), $extraConfig=false) {
        ...
        <input type="<?php echo $type; ?>"
            id="<?php echo $this->id; ?>"
            <?php echo $this->getInlineAttributes($config, array($placeholder)); ?>
...
class PhoneNumberWidget extends Widget {
    function render($options=array()) {
        ...
        if ($showExt) {
            $extName = $this->name."-ext";
            ?><div class="row"><div class="col-xs-7"><?php
        }
        ?>
        <input id="<?php echo $this->id; 
        ?>" type="tel" <?php
        echo $this->getInlineAttributes($config, array($placeholder));
        ?> name="<?php echo $this->name; 
        ?>" value="<?php
        ...
        if ($showExt) { ?>
            </div><div class="col-xs-5"><div class="row" style="display: inline-block"><div class="col-xs-3">
            <label class="control-label" for="<?php echo $extName; ?>"><?php echo __('Ext'); ?>:</label>
            </div><div class="col-xs-9">
            <input type="tel" <?php
            echo $this->getInlineAttributes(array('classes' => $config['classes']));
            ?> name="<?php
            echo $extName; ?>" value="<?php echo Format::htmlchars($ext);
                ?>" size="5"/>
            </div></div></div></div><?php
        }
    }
